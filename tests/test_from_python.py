...
def main() -> OutputMaterial:
    ...
    return Material_Output
        ...
        diffuse_node.add_input_socket(
            InputSocket("Color", diffuse_node, (0.800, 0.800, 0.800, 1.000))
        )
        diffuse_node.add_input_socket(InputSocket("Roughness", diffuse_node, 0))
        diffuse_node.add_input_socket(InputSocket("Normal", diffuse_node, (0, 0, 0)))
        diffuse_node.add_input_socket(InputSocket("Weight", diffuse_node, 0))
        ...
        output_node.add_input_socket(
            InputSocket("Displacement", output_node, (0, 0, 0))
        )
        output_node.add_input_socket(InputSocket("Thickness", output_node, 0))
